...
public class StatisticalAnalysis {
	...
	public double getSpearmansCorrelation(int combinationsize) {
		return new SpearmansCorrelation()
				.correlation(
						perturbationPanel
								.getObservedCombinationResponses(combinationsize),
						perturbationPanel
								.getPredictedAverageCombinationResponses(combinationsize));
	}
	...
	public double getKendallsCorrelation(int combinationsize) {
		return new KendallsCorrelation()
				.correlation(
						perturbationPanel
								.getObservedCombinationResponses(combinationsize),
						perturbationPanel
	...
	public double getSynergyClassificationAccuracy(int combinationsize) {
		...
		for (int i = 0; i < perturbationPanel
				.getNumberOfPerturbations(combinationsize); i++) {
			double predicted = perturbationPanel
					.getPerturbations(combinationsize)[i]
					.getAveragePredictedResponse();
			double observed = perturbationPanel
					.getPerturbations(combinationsize)[i].getObservedResponse();
	...
	/**
	 * http://stackoverflow.com/questions/718554/how-to-convert-an-arraylist-
