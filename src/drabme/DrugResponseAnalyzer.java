...
public class DrugResponseAnalyzer {
	...
	public DrugResponseAnalyzer(PerturbationPanel perturbationPanel, ArrayList <BooleanModel> booleanModels, ModelOutputs modelOutputs) {
	...
	public void analyze () throws IOException 
	{
		...
		{
			Logger.output(2, "Adding model " + booleanModels.get(modelIndex).getModelName()) ;
		
			ResponseModel rm = new ResponseModel(booleanModels.get(modelIndex), modelOutputs, perturbationPanel); 
			...
			for (int i = 0; i < perturbationPanel.getNumberOfPerturbations(); i++)
			{
				...
				for (int j = 0; j < perturbationPanel.getPerturbations()[i].getDrugs().length; j++)
				{
					
					if (Logger.getVerbosity() >= 2)
					{
						msg += perturbationPanel.getPerturbations()[i].getDrugs()[j].getName() + " ";
					}
					
				}
				Logger.output(2, "Added simulation with drug(s): " + msg + "to model: " + rm.getModelName()) ;
			}
